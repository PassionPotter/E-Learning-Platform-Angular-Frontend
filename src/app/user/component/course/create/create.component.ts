...
@Component({ selector: 'app-course-create', templateUrl: '../form.html' })
export class CourseCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.queryGrades();
    this.hashCourse = localStorage.getItem('hast_course');
    if (!this.hashCourse) {
      this.hashCourse = randomHash(32, '');
      localStorage.setItem('hast_course', this.hashCourse);
    }
  }

  ngOnDestroy() {
    if (this.hashCourse) {
      this.calendarService.deleteByHash(this.hashCourse);
      localStorage.removeItem('hast_course');
    }
  }
  queryCategories() {
    this.loading = true;
  ...
  submit(frm: any) {
    ...
    this.calendarService.checkByHash(this.hashCourse).then(check => {

      const data = this.hashCourse ? Object.assign(this.course, { hashCourse: this.hashCourse }) : this.course;
      this.courseService.create(data).then(
        resp => {
          localStorage.removeItem('hast_course');
          this.toasty.success(this.translate.instant('Course has been created'));
          this.router.navigate(['/users/courses/' + resp.data._id]);
          localStorage.setItem("from", "create")
        },
  ...
  addLectureSection() {
    const modalRef = this.modalService.open(ModalLectureSection, { centered: true, backdrop: 'static' });
  }
